        ...
        msg) 
{
  ...
  // std::cout << "ScheduledGaitCallback:\n";
  // std::cout << scheduledGaitSequence << std::endl;

  // std::string gaitSequenceStr;
  // gaitSequenceStr += "Received Gait Sequence: \n";
  // gaitSequenceStr += "   startTime: " + std::to_string(scheduledGaitSequence.first) + "\n";
  ...
  // for (size_t i = 0; i < scheduledGaitSequence.second.size(); i++)
  // {
  //     gaitSequenceStr += "   Gait " + std::to_string(i) + ":\n";
  //     gaitSequenceStr += "       duration = " + std::to_string(scheduledGaitSequence.second[i].duration) + "\n";
  //     gaitSequenceStr += "       modeSequence: ";
  //     for (size_t j = 0; j < scheduledGaitSequence.second[i].modeSequence.size(); j++)
  //     {
  //         gaitSequenceStr += switched_model::modeNumber2String(scheduledGaitSequence.second[i].modeSequence[j]) + " \n";
  //     }
  //     gaitSequenceStr += "\n";
  //     gaitSequenceStr += "       eventPhases: ";
  //     for (size_t j = 0; j < scheduledGaitSequence.second[i].eventPhases.size(); j++)
  //     {
  //         gaitSequenceStr += std::to_string(scheduledGaitSequence.second[i].eventPhases[j]) + "\n";
  //     }
  //     gaitSequenceStr += "\n";

  //     // RCLCPP_INFO_STREAM(node_->get_logger(), "           duration = " << scheduledGaitSequence.second[i].duration);
  //     // for (size_t j = 0; j < scheduledGaitSequence.second[i].modeSequence.size(); j++)
  //     // {
  //     //     RCLCPP_INFO_STREAM(node_->get_logger(), "           mode " << j << ": " << switched_model::modeNumber2String(scheduledGaitSequence.second[i].modeSequence[j]));
  //     // }
  //     // RCLCPP_INFO_STREAM(node_->get_logger(), "           eventPhases: ");
  //     // for (size_t j = 0; j < scheduledGaitSequence.second[i].eventPhases.size(); j++)
  //     // {
  //     //     RCLCPP_INFO_STREAM(node_->get_logger(), "               " << scheduledGaitSequence.second[i].eventPhases[j]);
  //     // }
  // }
  // RCLCPP_INFO(node_->get_logger(), "%s", gaitSequenceStr.c_str());
}
