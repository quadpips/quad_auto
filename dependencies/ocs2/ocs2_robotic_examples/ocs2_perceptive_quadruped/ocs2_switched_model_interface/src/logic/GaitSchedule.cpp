...
{
  // std::cout << "[GaitSchedule::advanceToTime] Advancing to time: " << time << std::endl;
  if (time <= time_) {
    return;
  }

  // std::cout << "    phase_: " << phase_ << std::endl;
  // std::cout << "gaitSchedule_: " << std::endl;
  // for (const auto & gait : gaitSchedule_)
...
{
  // std::cout << "[GaitSchedule::setGaitSequenceAfterTime]" << std::endl;

  assert(time >= time_);

  rolloutGaitScheduleTillTime(time);

  // std::cout << "      gaitSchedule_ (post rollout): " << gaitSchedule_ << std::endl;

  scalar_t newPhase;
  std::deque<Gait>::iterator newActiveGait;
  std::tie(newPhase, newActiveGait) = advancePhase(phase_, time - time_, gaitSchedule_.begin(), gaitSchedule_.end());

  // std::cout << "      gaitSequence: " << gaitSequence << std::endl;

  gaitSchedule_.erase(newActiveGait + 1, gaitSchedule_.end());
  gaitSchedule_.insert(gaitSchedule_.end(), gaitSequence.begin(), gaitSequence.end());

  // std::cout << "      gaitSchedule_ (post-insertion): " << gaitSchedule_ << std::endl;
...
{
  // std::cout << "[GaitSchedule::getModeSchedule]" << std::endl;
