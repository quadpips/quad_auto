...
struct SwingTrajectoryPlannerSettings {
  ...
  bool setModeScheduleFromGait = false; // Flag for if you should set mode schedule based on gait schedule or not
};
...
class SwingTrajectoryPlanner {
 public:
  ...
  const TerrainModel& getTerrainModel() const { return *terrainModel_; } 
