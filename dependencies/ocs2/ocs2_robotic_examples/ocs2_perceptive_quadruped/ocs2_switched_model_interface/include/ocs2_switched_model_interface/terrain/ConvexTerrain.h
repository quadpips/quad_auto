...
/**
 * Adapted from: https://math.stackexchange.com/questions/274712/calculate-on-which-side-of-a-straight-line-is-a-given-point-located
 * Checks if a 2D point lies within the boundary of a 2D convex polygon.
 * @param [in] boundary: The vertices of the polygon in clockwise or counter-clockwise order.
 * @param [in] p: The 2D point.
 * @return Boolean for whether the point is within the boundary.
 */
inline bool isPointWithinConvex2dPolygonBoundary(const std::vector<vector2_t>& boundary, const vector2_t& p) 
{
  bool isInside = true;
  ...
  for (int i = 0; i < boundary.size(); i++) 
  {
    ...
    if (p1.x() == p.x() && p1.y() == p.y()) // edge case: query point is a vertex of the polygon 
      return true;

    const scalar_t cross_product = (p.x() - p1.x()) * (p2.y() - p1.y()) - (p.y() - p1.y()) * (p2.x() - p1.x());
...
/**
 * THIS IS WRONG AND DOES NOT DO WHAT IS SAYS IT DOES.
