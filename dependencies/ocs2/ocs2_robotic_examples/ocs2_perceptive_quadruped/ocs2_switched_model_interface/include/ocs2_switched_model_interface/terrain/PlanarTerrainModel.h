...
class PlanarTerrainModel : public TerrainModel {
 public:
  ...
  std::vector<ConvexTerrain> getAllConvexTerrains() const override { std::vector<ConvexTerrain> terrains; return terrains; }  

  scalar_t getHeightAtPosition(const vector3_t& positionInWorld) const override { return 0.0; }

  const grid_map::GridMap * getGridMap() const override { return nullptr; }
