 ...
 */
class CustomQuadrupedLogger : public ocs2::DummyObserver {
 public:
  using com_model_t = ComModelBase<double>;
  using kinematic_model_t = KinematicsModelBase<double>;

  CustomQuadrupedLogger(std::string logFileName, const kinematic_model_t& kinematicModel, const com_model_t& comModel,
                  std::vector<std::string> additionalColumns = {});

  /**
   * The destructor writes the buffered log data to file.
   */
  ~CustomQuadrupedLogger() override;
