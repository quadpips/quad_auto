...
class CustomQuadrupedVisualizer : public ocs2::DummyObserver {
 public:
   ...
   */
  CustomQuadrupedVisualizer(const kinematic_model_t& kinematicModel,
                            std::vector<std::string> jointNames, 
                            std::string baseName,
                            const rclcpp::Node::SharedPtr& node,
                            scalar_t maxUpdateFrequency = 1000.0,
                            const bool & includeTFs = true);

  ~CustomQuadrupedVisualizer() override = default;
 ...
 private:
  ...
  void publishBaseFrame(rclcpp::Time timeStamp, 
        const vector3_t& o_basePosition, const vector3_t& o_baseOrientation) const;
  ...
  CustomQuadrupedTfPublisher quadrupedTfPublisher_;
