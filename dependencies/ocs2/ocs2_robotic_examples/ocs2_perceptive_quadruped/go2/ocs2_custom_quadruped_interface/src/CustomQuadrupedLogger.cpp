...
CustomQuadrupedLogger::CustomQuadrupedLogger(std::string logFileName, const kinematic_model_t& kinematicModel, const com_model_t& comModel,
...
CustomQuadrupedLogger::~CustomQuadrupedLogger() {
  ...
  if (std::ofstream logfile{logFileName_}) 
  {
    ...
    std::cerr << "[CustomQuadrupedLogger] Log written to '" << logFileName_ << "'\n";
  } else {
    std::cerr << "[CustomQuadrupedLogger] Unable to open '" << logFileName_ << "'\n";
  }
}

std::string CustomQuadrupedLogger::getLogHeader() const {
  std::string delim = ", ";
...
int CustomQuadrupedLogger::getNumColumns() const {
  int numColumns = 0;
  numColumns += 1;                          // time
...
void CustomQuadrupedLogger::addLine(const ocs2::SystemObservation& observation, const vector_t& additionalColumns) {
...
std::vector<std::string> CustomQuadrupedLogger::namesPerLeg(const std::string& prefix, const std::vector<std::string>& postfixes) {
