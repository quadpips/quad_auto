...
CustomQuadrupedVisualizer::CustomQuadrupedVisualizer(const kinematic_model_t& kinematicModel,
                                                      std::vector<std::string> jointNames, 
                                                      std::string baseName,
                                                      const rclcpp::Node::SharedPtr& node, 
                                                      scalar_t maxUpdateFrequency,
                                                      const bool & includeTFs)
    : node_(node),
      kinematicModelPtr_(kinematicModel.clone()),
      lastTime_(std::numeric_limits<scalar_t>::lowest()),
      minPublishTimeDifference_(1.0 / maxUpdateFrequency),
      includeTFs_(includeTFs)  
{
  costDesiredBasePositionPublisher_ =
      node->create_publisher<visualization_msgs::msg::Marker>(
          "/ocs2_go2/desiredBaseTrajectory", 1);
  costDesiredBasePosePublisher_ =
      node->create_publisher<geometry_msgs::msg::PoseArray>(
          "/ocs2_go2/desiredPoseTrajectory", 1);
  costDesiredBaseAngVelocityPublisher_ =
      node->create_publisher<visualization_msgs::msg::MarkerArray>(
          "/ocs2_go2/desiredAngVelTrajectory", 1);
  costDesiredBaseVelocityPublisher_ =
      node->create_publisher<visualization_msgs::msg::MarkerArray>(
          "/ocs2_go2/desiredVelTrajectory", 1);
  costDesiredFeetPositionPublishers_[0] =
      node->create_publisher<visualization_msgs::msg::Marker>(
          "/ocs2_go2/desiredFeetTrajectory/LF", 1);
  costDesiredFeetPositionPublishers_[1] =
      node->create_publisher<visualization_msgs::msg::Marker>(
          "/ocs2_go2/desiredFeetTrajectory/RF", 1);
  costDesiredFeetPositionPublishers_[2] =
      node->create_publisher<visualization_msgs::msg::Marker>(
          "/ocs2_go2/desiredFeetTrajectory/LH", 1);
  costDesiredFeetPositionPublishers_[3] =
      node->create_publisher<visualization_msgs::msg::Marker>(
          "/ocs2_go2/desiredFeetTrajectory/RH", 1);
  costDesiredFeetVelocityPublishers_[0] =
      node->create_publisher<visualization_msgs::msg::MarkerArray>(
          "/ocs2_go2/desiredFeetVelTrajectory/LF", 1);
  costDesiredFeetVelocityPublishers_[1] =
      node->create_publisher<visualization_msgs::msg::MarkerArray>(
          "/ocs2_go2/desiredFeetVelTrajectory/RF", 1);
  costDesiredFeetVelocityPublishers_[2] =
      node->create_publisher<visualization_msgs::msg::MarkerArray>(
          "/ocs2_go2/desiredFeetVelTrajectory/LH", 1);
  costDesiredFeetVelocityPublishers_[3] =
      node->create_publisher<visualization_msgs::msg::MarkerArray>(
          "/ocs2_go2/desiredFeetVelTrajectory/RH", 1);
  stateOptimizedPublisher_ =
      node->create_publisher<visualization_msgs::msg::MarkerArray>(
          "/ocs2_go2/optimizedStateTrajectory", 1);
  stateOptimizedPosePublisher_ =
      node->create_publisher<geometry_msgs::msg::PoseArray>(
          "/ocs2_go2/optimizedPoseTrajectory", 1);
  currentStatePublisher_ =
      node->create_publisher<visualization_msgs::msg::MarkerArray>(
          "/ocs2_go2/currentState", 1);
  currentFeetPosesPublisher_ =
      node->create_publisher<geometry_msgs::msg::PoseArray>(
          "/ocs2_go2/currentFeetPoses", 1);
  currentCollisionSpheresPublisher_ =
      node->create_publisher<visualization_msgs::msg::MarkerArray>(
          "/ocs2_go2/currentCollisionSpheres", 1);

  if (includeTFs_) 
  {
    quadrupedTfPublisher_.launchNode(node, std::move(jointNames), std::move(baseName));
  } else
  {
    RCLCPP_DEBUG_STREAM(node_->get_logger(), "QuadrupedVisualizer: TFs are not published.");
  }
}

void CustomQuadrupedVisualizer::update(const ocs2::SystemObservation& observation,
                                        const ocs2::PrimalSolution& primalSolution,
                                        const ocs2::CommandData& command) 
{
  // RCLCPP_INFO_STREAM(node_->get_logger(), "CustomQuadrupedVisualizer::update()");
  if (observation.time - lastTime_ > minPublishTimeDifference_) 
  {
    // RCLCPP_INFO_STREAM(node_->get_logger(), "Publishing observation");
  ...
  // else
  // {
    // RCLCPP_DEBUG_STREAM(node_->get_logger(), "Skipping observation publishing");
  // }
}
void CustomQuadrupedVisualizer::publishObservation(
    rclcpp::Time timeStamp, const ocs2::SystemObservation& observation) 
{
  ...
  const auto o_baseOrientation = getOrientation(basePose);

  const Eigen::Matrix3d o_R_b =
      rotationMatrixBaseToOrigin<scalar_t>(o_baseOrientation);
  ...
  {
    std::string footName = feetNames[i];
  ...
  if (includeTFs_)
  {
    // RCLCPP_INFO_STREAM(node_->get_logger(), "Publishing TFs");
...
void CustomQuadrupedVisualizer::publishTrajectory(
...
void CustomQuadrupedVisualizer::publishCartesianMarkers(
...
void CustomQuadrupedVisualizer::publishDesiredTrajectory(
...
void CustomQuadrupedVisualizer::publishOptimizedStateTrajectory(
...
void CustomQuadrupedVisualizer::publishEndEffectorPoses(
...
void CustomQuadrupedVisualizer::publishCollisionSpheres(
