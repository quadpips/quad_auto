...
{
  CustomSwingPlanningVisualizer::CustomSwingPlanningVisualizer(const SwingTrajectoryPlanner &swingTrajectoryPlanner, const rclcpp::Node::SharedPtr &node)
      : node_(node), swingTrajectoryPlannerPtr_(&swingTrajectoryPlanner)
  {
    nominalFootholdPublishers_[0] = node->create_publisher<geometry_msgs::msg::PoseArray>("/ocs2_go2/swing_planner/nominalFootholds_LF", 1);
    nominalFootholdPublishers_[1] = node->create_publisher<geometry_msgs::msg::PoseArray>("/ocs2_go2/swing_planner/nominalFootholds_RF", 1);
    nominalFootholdPublishers_[2] = node->create_publisher<geometry_msgs::msg::PoseArray>("/ocs2_go2/swing_planner/nominalFootholds_LH", 1);
    nominalFootholdPublishers_[3] = node->create_publisher<geometry_msgs::msg::PoseArray>("/ocs2_go2/swing_planner/nominalFootholds_RH", 1);
    swingTrajectoryPublishers_[0] = node->create_publisher<visualization_msgs::msg::MarkerArray>("/ocs2_go2/swing_planner/trajectory_LF", 1);
    swingTrajectoryPublishers_[1] = node->create_publisher<visualization_msgs::msg::MarkerArray>("/ocs2_go2/swing_planner/trajectory_RF", 1);
    swingTrajectoryPublishers_[2] = node->create_publisher<visualization_msgs::msg::MarkerArray>("/ocs2_go2/swing_planner/trajectory_LH", 1);
    swingTrajectoryPublishers_[3] = node->create_publisher<visualization_msgs::msg::MarkerArray>("/ocs2_go2/swing_planner/trajectory_RH", 1);
  }

  void CustomSwingPlanningVisualizer::preSolverRun(scalar_t initTime, scalar_t finalTime, const vector_t &currentState,
                                             const ocs2::ReferenceManagerInterface &referenceManager)
  {
    ...
    // std::cout << "[CustomSwingPlanningVisualizer::preSolverRun] publishing swing trajectories" << std::endl;
    for (int leg = 0; leg < NUM_CONTACT_POINTS; leg++)
    {
      ...
      for (const auto &t : swingTrajectoryPlannerPtr_->getTargetTrajectories().timeTrajectory)
      {
        ...
        // std::cout << "            position: " << node.position.transpose() << std::endl;
        // std::cout << "            velocity: " << node.velocity.transpose() << std::endl;
