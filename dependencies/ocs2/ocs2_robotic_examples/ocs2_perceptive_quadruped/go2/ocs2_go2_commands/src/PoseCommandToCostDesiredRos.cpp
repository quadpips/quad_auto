...
#include "ocs2_go2_commands/PoseCommandToCostDesiredRos.h"

#include <ocs2_core/misc/LoadData.h>

#include "ocs2_go2_commands/TerrainAdaptation.h"
...
namespace switched_model
{
  ...
  PoseCommandToCostDesiredRos::PoseCommandToCostDesiredRos(const rclcpp::Node::SharedPtr &node, const std::string &configFile)
  {
    ...
    terrainSubscriber_ = node->create_subscription<visualization_msgs::msg::Marker>("/ocs2_go2/localTerrain", 1, std::bind(&PoseCommandToCostDesiredRos::terrainCallback, this, std::placeholders::_1));
  }
                                                                                        ...
                                                                                        const ocs2::SystemObservation &observation) const
  {
    ...
    vector3_t comPositionDesired{commadLineTarget(0) + observation.state[3], 
                                 commadLineTarget(1) + observation.state[4], 
                                 commadLineTarget(2) + comHeight};
    // Roll and pitch are absolute, yaw is relative
    vector3_t comOrientationDesired{deg2rad(commadLineTarget(3)), 
                                    deg2rad(commadLineTarget(4)), 
                                    deg2rad(commadLineTarget(5)) + observation.state[2]};
