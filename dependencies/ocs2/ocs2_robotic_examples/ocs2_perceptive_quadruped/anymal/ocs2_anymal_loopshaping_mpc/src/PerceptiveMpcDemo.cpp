...
int main(int argc, char* argv[]) {
    ...
    rclcpp::Node::SharedPtr node =
        rclcpp::Node::make_shared("ocs2_anymal_loopshaping_mpc_perceptive_demo",
                rclcpp::NodeOptions()
...
//   node->declare_parameter("config_name", "c_series");
    configName = node->get_parameter("config_name").as_string();

    convex_plane_decomposition::PlaneDecompositionPipeline::Config
        perceptionConfig;
    perceptionConfig.preprocessingParameters =
        convex_plane_decomposition::loadPreprocessingParameters(node.get(), "preprocessing/");
    perceptionConfig.contourExtractionParameters =
        convex_plane_decomposition::loadContourExtractionParameters(node.get(), "contour_extraction/");
    perceptionConfig.ransacPlaneExtractorParameters =
        convex_plane_decomposition::loadRansacPlaneExtractorParameters(node.get(), "ransac_plane_refinement/");
    perceptionConfig.slidingWindowPlaneExtractorParameters =
        convex_plane_decomposition::loadSlidingWindowPlaneExtractorParameters(node.get(), "sliding_window_plane_extractor/");
    perceptionConfig.postprocessingParameters =
        convex_plane_decomposition::loadPostprocessingParameters(node.get(), "postprocessing/");
    ...
    std::cout << "init system state: " << initSystemState.transpose() << "\n";
