...
namespace switched_model
{
      ...
      : node_(node), swingTrajectoryPlannerPtr_(&swingTrajectoryPlanner)
  {
    ...
    swingTrajectoryPublishers_[0] = node->create_publisher<visualization_msgs::msg::Marker>("/ocs2_anymal/swing_planner/trajectory_LF", 1);
    swingTrajectoryPublishers_[1] = node->create_publisher<visualization_msgs::msg::Marker>("/ocs2_anymal/swing_planner/trajectory_RF", 1);
    swingTrajectoryPublishers_[2] = node->create_publisher<visualization_msgs::msg::Marker>("/ocs2_anymal/swing_planner/trajectory_LH", 1);
    swingTrajectoryPublishers_[3] = node->create_publisher<visualization_msgs::msg::Marker>("/ocs2_anymal/swing_planner/trajectory_RH", 1);
  }
                                             ...
                                             const ocs2::ReferenceManagerInterface &referenceManager)
  {
    ...
    for (int leg = 0; leg < NUM_CONTACT_POINTS; leg++)
    {
      ...
      visualization_msgs::msg::Marker swingTrajectoriesMessage;
      ...
      for (const auto &t : swingTrajectoryPlannerPtr_->getTargetTrajectories().timeTrajectory)
      {
        ...
        geometry_msgs::msg::Point footPoint = ocs2::getPointMsg(node.position);
        swingTrajectoriesMessage.points.push_back(footPoint);
      ...
      swingTrajectoriesMessage.type = visualization_msgs::msg::Marker::LINE_STRIP;
      swingTrajectoriesMessage.scale.x = 0.02; // line width
      swingTrajectoriesMessage.color = ocs2::getColor(feetColorMap_[leg]);
      swingTrajectoriesMessage.header = ocs2::getHeaderMsg(frameId_, timeStamp);
      swingTrajectoriesMessage.ns = "Swing Trajectory";
      swingTrajectoriesMessage.id = 0;
