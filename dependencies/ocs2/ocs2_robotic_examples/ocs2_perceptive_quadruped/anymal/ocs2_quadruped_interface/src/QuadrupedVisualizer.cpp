...
QuadrupedVisualizer::QuadrupedVisualizer(const kinematic_model_t& kinematicModel,
...
void QuadrupedVisualizer::visualizeWBCContactForces(const ocs2::SystemObservation& observation,
                                                    const vector_t & wbcForces)
{
  ...
  for (size_t i = 0; i < NUM_CONTACT_POINTS; i++) 
  {
    ...
    marker = getArrowAtPointMsg(feetForce[i] / forceScale_, feetPosition[i], ocs2::Color::red);
    marker.header.frame_id = frameId_;
    marker.header.stamp = timeStamp;
    marker.id = i;
    marker.ns = "WBC Forces";
    marker.type = visualization_msgs::msg::Marker::ARROW;
    marker.action = visualization_msgs::msg::Marker::ADD;
    markerArray.markers.push_back(marker);
  }

  wbcForcesPublisher_->publish(markerArray);
}
