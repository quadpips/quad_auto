...
SegmentedPlanesTerrainModelRos::SegmentedPlanesTerrainModelRos(
    ...
    : node_(node),
      ...
      vis_(vis)
{
  // std::cout << "[SegmentedPlanesTerrainModelRos] Constructor called." << std::endl;

  // std::cerr << "[SegmentedPlanesTerrainModelRos] IDKIDKIKD" << std::endl;

  // RCLCPP_INFO_STREAM(node->get_logger(), "[SegmentedPlanesTerrainModelRos] Constructor called.");
  ...
  // mmpTerrainUpdateSubscriber_ = node->create_subscription<
  //     std_msgs::msg::Int8>(
  //     "/mmp_terrain/update_notification", 1,
  //     [this](const std_msgs::msg::Int8::ConstSharedPtr msg) 
  //     {
  //       if (msg->data == 1) 
  //       {
  //         this->mmpUpdateTerrain_ = true;
  //       } else if (msg->data == 0) 
  //       {
  //         this->mmpUpdateTerrain_ = false;
  //       } else
  //       {
  //         RCLCPP_WARN_STREAM(node_->get_logger(), "[SegmentedPlanesTerrainModelRos] Received unknown MMP terrain update notification: " << msg->data);
  //       }
  //     });
        ...
        msg) 
{
  ...
  // if (!mmpUpdateTerrain_) 
  // {
  //   callbackTimer_.endTimer();
  //   return;
  // }

  // RCLCPP_INFO_STREAM(node_->get_logger(), "[SegmentedPlanesTerrainModelRos::callback]");
  // RCLCPP_INFO_STREAM(node_->get_logger(), "   Received terrain with " << msg->planar_regions.size() << " planar regions");
  
  // Read terrain
  std::unique_ptr<SegmentedPlanesTerrainModel> terrainPtr = std::make_unique<SegmentedPlanesTerrainModel>(convex_plane_decomposition::fromMessage(*msg));

  // Create SDF
  const std::string elevationLayer = "elevation";
  if (terrainPtr->planarTerrain().gridMap.exists(elevationLayer)) 
  {
    // RCLCPP_WARN_STREAM(node_->get_logger(), "   Elevation layer found in received terrain map.");

    const auto& elevationData = terrainPtr->planarTerrain().gridMap.get(elevationLayer);
    const float minValue = elevationData.minCoeffOfFinites();
    const float maxValue = elevationData.maxCoeffOfFinites();

    // RCLCPP_INFO_STREAM(node_->get_logger(), "   Elevation layer min: " << minValue << ", max: " << maxValue);
  ...
  // else
  // {
    // RCLCPP_WARN_STREAM(node_->get_logger(), "   Elevation layer not found in received terrain map.");
  // }
  ...
  // std::cout << "        terrainPtr_->planarTerrain().planarRegions.size(): " << terrainPtr_->planarTerrain().planarRegions.size() << std::endl;
