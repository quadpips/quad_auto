...
class SegmentedPlanesTerrainModel : public switched_model::TerrainModel {
 public:
  ...
  std::vector<ConvexTerrain> getAllConvexTerrains() const override;

  scalar_t getHeightAtPosition(const vector3_t& positionInWorld) const override;

  const grid_map::GridMap * getGridMap() const override { return &planarTerrain_.gridMap; }
